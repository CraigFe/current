...
let set_confirm t level =
  Log.info (fun f ->
      f "Confirmation threshold is now %a" (Fmt.Dump.option Level.pp) level);
  t.confirm <- level;
...
let slow_start_thread t duration =
  Lwt.async (fun () ->
      ...
      Lwt.choose [ Lwt_unix.sleep (Duration.to_f duration); changed ]
      >|= fun () ->
...
let rec confirmed l t =
  ...
  | Some threshold when Level.compare l threshold >= 0 ->
      Lwt_condition.wait t.level_cond >>= fun () -> confirmed l t
  | _ -> Lwt.return_unit
