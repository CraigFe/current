...
type t = {
  ...
  log_cond : unit Lwt_condition.t;
      (* Fires whenever log data is written, or log is closed. *)
}
...
let open_temp_file ~dir ~prefix ~suffix =
  let path, ch =
    Filename.open_temp_file ~temp_dir:(Fpath.to_string dir) prefix suffix
  in
  (Fpath.v path, ch)
...
let write t msg =
  match t.ch with
  | None ->
      Log.err (fun f -> f "Job.write(%s, %S) called on closed job" t.id msg)
  | Some ch ->
...
let log t fmt =
  let { Unix.tm_year; tm_mon; tm_mday; tm_hour; tm_min; tm_sec; _ } =
    !timestamp () |> Unix.gmtime
  in
  let fmt = "%04d-%02d-%02d %02d:%02d.%02d: @[" ^^ fmt ^^ "@]@." in
  Fmt.kstrf (write t) fmt (tm_year + 1900) (tm_mon + 1) tm_mday tm_hour tm_min
    tm_sec
...
let id_of_path path =
  ...
  | parent_dir, leaf ->
      Fpath.(base parent_dir // leaf)
      |> Fpath.to_string |> Filename.chop_extension

let create ~switch ~label ~config () =
  if not (Switch.is_on switch) then
    Fmt.failwith "Switch %a is not on! (%s)" Switch.pp switch label;
  let jobs_dir = Lazy.force jobs_dir in
  ...
  | Ok (_ : bool) ->
      ...
      Switch.add_hook_or_fail switch (fun reason ->
          (match reason with Ok () -> () | Error (`Msg m) -> log t "%s" m);
          close_out ch;
          t.ch <- None;
...
let confirm t level =
  ...
  Switch.add_hook_or_fail t.switch (fun _ ->
      Lwt.cancel confirmed;
      Lwt.return_unit);
