...
let or_fail ~cmd x =
  ...
  | Sqlite3.Rc.OK -> ()
  | err ->
      Fmt.failwith "Sqlite3: %s (executing %S)" (Sqlite3.Rc.to_string err) cmd
...
let exec_literal db sql = Sqlite3.exec db sql |> or_fail ~cmd:sql

let bind stmt values =
  Sqlite3.reset stmt |> or_fail ~cmd:"reset";
  List.iteri
    (fun i v -> Sqlite3.bind stmt (i + 1) v |> or_fail ~cmd:"bind")
    values
