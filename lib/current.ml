...
module Step = struct
  ...
  let register_job t id actions = t.jobs <- Job_map.add id actions t.jobs
...
module Input = struct
  ...
  let metadata ?job_id ?changed actions = { job_id; changed; actions }
...
module Var (T : Current_term.S.T) = struct
  ...
  let create ~name current = { current; name; cond = Lwt_condition.create () }
...
let rec filter_map f = function
  | [] -> []
  | x :: xs -> (
      match f x with None -> filter_map f xs | Some y -> y :: filter_map f xs )
...
module Engine = struct
  ...
  let default_trace r =
    Log.info (fun f ->
        f "Result: %a" Current_term.(Output.pp Fmt.(unit "()")) r.value);
    Lwt.return_unit
  ...
  let create ?(config = Config.default) ?(trace = default_trace) f =
    ...
    let rec aux () =
      ...
      last_result :=
        { value = r; analysis = an; watches; jobs = step.Step.jobs };
      trace !last_result >>= fun () ->
      Log.debug (fun f -> f "Waiting for inputs to change...");
      Lwt.choose (filter_map (fun w -> w.changed) watches) >>= fun () -> aux ()
    in
    let thread =
  ...
  let is_stale m =
    match m.changed with Some p -> Lwt.state p <> Lwt.Sleep | None -> false
end
...
module Monitor : sig
  ...
  type 'a t = {
    ...
    (* Number of terms using this input *)
    mutable need_refresh : bool;
    (* Update detected after current read started *)
    mutable active : bool;
    (* Monitor thread is running *)
    cond : unit Lwt_condition.t;
    (* Maybe time to leave the "wait" state *)
    external_cond : unit Lwt_condition.t;
        (* New value ready for external user *)
  }
  ...
  let rec enable t =
    ...
    if t.ref_count = 0 then disable ~unwatch t else get_value t ~unwatch
