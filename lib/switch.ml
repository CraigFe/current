(** Like [Lwt_switch], but the cleanup functions are called in sequence, not in
    parallel, and a reason for the shutdown may be given. *)
...
type t = {
  mutable state :
    [ `On of string * callback Stack.t
    | `Turning_off of reason Lwt.t
    | `Off of reason ];
}

let pp_reason f x =
  Current_term.Output.pp (Fmt.unit "()") f (x :> unit Current_term.Output.t)

let turn_off t reason =
  match t.state with
  | `Off orig ->
      Log.info (fun f ->
          f "Switch.turn_off(%a): already off (due to %a)" pp_reason reason
            pp_reason orig);
      Lwt.return_unit
  | `Turning_off thread -> thread >|= fun (_ : reason) -> ()
  | `On (_, callbacks) ->
...
let gc t =
  ...
  | `On (label, _) ->
      ...
      Lwt.async (fun () ->
          turn_off t @@ Error (`Msg "Switch GC'd while still on!"))
...
let add_hook_or_exec t fn =
  ...
  | `On (_, callbacks) ->
      ...
      Stack.push fn callbacks;
      Lwt.return_unit
  | `Off reason -> fn reason
  | `Turning_off thread -> thread >>= fn

let add_hook_or_exec_opt t fn =
  match t with None -> Lwt.return_unit | Some t -> add_hook_or_exec t fn

let create ~label () = { state = `On (label, Stack.create ()) }

let create_off reason = { state = `Off reason }

let is_on t =
  match t.state with `On _ -> true | `Off _ | `Turning_off _ -> false
...
let add_timeout t duration =
  ...
  Lwt.async (fun () ->
      ...
      if is_on t then
        turn_off t
        @@ Error (`Msg (Fmt.strf "Timeout (%a)" pp_duration duration))
