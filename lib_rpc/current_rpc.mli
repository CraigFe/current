module Job : sig
  ...
  val log :
    start:int64 ->
    t ->
    (string * int64, [> `Capnp of Capnp_rpc.Error.t ]) result Lwt.t
...
module Engine : sig
  ...
  val active_jobs :
    t -> (Job.id list, [> `Capnp of Capnp_rpc.Error.t ]) result Lwt.t
  (** [active_jobs t] lists the OCurrent jobs that are still being used in the
      pipeline. This includes completed jobs, as long as OCurrent is still
      ensuring they are up-to-date. *)

  val job : t -> Job.id -> Job.t
  (** [job t id] is the job with the given ID. This does not have to be an
      active job (but only active jobs can be rebuilt). If the job ID is
      unknown, this operation will resolve to a suitable error. *)
end

module Impl (Current : S.CURRENT) : sig
  (** This is used on the server-side to provide access to the OCurrent Engine.
      Create an instance of the functor with
      [module Rpc = Current_rpc.Impl(Current)]. We use a functor here just to
      avoid having Current_rpc depend on Current, which would be annoying for
      RPC clients. *)
