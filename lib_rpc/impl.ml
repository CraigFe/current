...
module Make (Current : S.CURRENT) = struct
  ...
  module Job = struct
    ...
    let stream_log_data ~job_id ~start =
      ...
      | Error (`Msg m) ->
          Lwt_result.fail (`Exception (Capnp_rpc.Exception.v m))
      | Ok path ->
          let rec aux () =
          ...
          let cap =
            ...
            @@ object
                 ...
                 method rebuild_impl _params release_param_caps =
                   ...
                   match lookup () with
                   | None ->
                       Service.fail "Job is no longer active (cannot rebuild)"
                   | Some job -> (
                       match job#rebuild with
                       | None ->
                           Service.fail "Job cannot be rebuilt at the moment"
                       | Some rebuild ->
                 ...
                 method cancel_impl _params release_param_caps =
                   ...
                   match lookup () with
                   | None ->
                       Service.fail "Job is no longer active (cannot cancel)"
                   | Some job -> (
