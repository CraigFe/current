...
module Fetch = struct
  ...
  let build ~switch No_context job key =
    ...
    (Commit.check_cached ~job commit >>= function
     | Ok () -> Lwt.return (Ok ())
     | Error _ ->
         Cmd.git_fetch ~switch ~job ~src:remote_repo ~dst:local_repo gref)
    >>!= fun () ->
    (* Check we got the commit we wanted. *)
    Commit.check_cached ~job commit >>!= fun () -> Lwt.return @@ Ok commit
...
let with_checkout ~switch ~job commit fn =
  ...
  Current.Job.log job
    "@[<v2>Checking out commit %s. To reproduce:@,\
     git clone %S -b %S && cd %S && git reset --hard %s@]"
    short_hash id.Commit_id.repo
...
module Local = struct
  ...
  let make_input t gref =
    ...
    let watch refresh =
      let watch_dir =
        Fpath.append dot_git (Fpath.v @@ Filename.dirname gref)
      in
      ...
      Irmin_watcher.hook (next_id ()) (Fpath.to_string watch_dir) (fun path ->
          if path = Filename.basename gref then (
            ...
            refresh () )
          else Log.debug (fun f -> f "Ignoring change in %S" path);
          Lwt.return_unit)
      >|= fun unwatch ->
      ...
      fun () ->
        ...
        unwatch ()
    in
    let pp f = Fmt.pf f "%a#%s" pp_repo t gref in
  ...
  let read_head repo =
    ...
    | Ok contents -> (
        ...
        | [ hash ] ->
            Ok
              (`Commit
                { Commit_id.repo = Fpath.to_string repo; gref = "HEAD"; hash })
        | [ _; r ] -> Ok (`Ref r)
  ...
  let make_head_input repo =
    ...
    let watch refresh =
      ...
      fun () ->
        ...
        unwatch ()
    in
    let pp f = Fmt.pf f "HEAD(%a)" Fpath.pp repo in
    Current.monitor ~read ~watch ~pp

  let v repo =
    let repo =
      Fpath.normalize @@ Fpath.append (Fpath.v (Sys.getcwd ())) repo
    in
