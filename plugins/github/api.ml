...
module Metrics = struct
  ...
  let used_points_total =
    ...
    Counter.v_label ~label_name:"account" ~help ~namespace ~subsystem
      "used_points_total"
end
...
let status_endpoint ~owner_name ~commit =
  Uri.of_string
    (Fmt.strf "https://api.github.com/repos/%s/statuses/%s" owner_name commit)
...
module Status = struct
  ...
  type t = { state : state; description : string option; url : Uri.t option }

  let v ?description ?url state =
    let description =
      Option.map (Astring.String.with_range ~len:140) description
    in
    (* Max GitHub allows *)
  ...
  let json_items { state; description; url } =
    ...
    @ ( match description with
      | None -> []
      | Some x -> [ ("description", `String x) ] )
    @
    match url with
    | None -> []
    | Some x -> [ ("target_url", `String (Uri.to_string x)) ]
...
let no_token = { token = Error (`Msg "Not fetched yet"); expiry = Some (-1.0) }
...
module Commit_id = struct
  ...
  let pp f { owner_name; id; hash } =
    Fmt.pf f "@[<v>%s@,%a@,%s@]" owner_name pp_id id
...
type t = {
  account : string;
  (* Prometheus label used to report points. *)
  ...
  | Ok token -> (
      ...
      | `OK -> (
          ...
          | errors -> (
              ...
              | _ -> Fmt.failwith "Unknown error type from GitHub GraphQL" ) )
      | err ->
          Fmt.failwith
...
let query_default =
  "query($owner: String!, $name: String!) { rateLimit { cost remaining \
   resetAt } repository(owner: $owner, name: $name) { nameWithOwner \n\n\
  \     defaultBranchRef { prefix name target { oid } } } }"
...
let handle_rate_limit t name json =
  ...
  Log.info (fun f ->
      f "GraphQL(%s): cost:%d remaining:%d resetAt:%s" name cost remaining
        reset_at);
  Prometheus.Counter.inc
    (Metrics.used_points_total t.account)
    (float_of_int cost);
  Prometheus.Gauge.set
    (Metrics.remaining_points t.account)
    (float_of_int remaining)

let default_ref t { Repo_id.owner; name } =
  let variables = [ ("owner", `String owner); ("name", `String name) ] in
  ...
  let read () =
    Lwt.catch
      ...
      (fun ex ->
        Lwt_result.fail
        @@ `Msg
             (Fmt.strf "GitHub query for %a failed: %a" Repo_id.pp repo Fmt.exn
                ex))
  in
  let watch refresh =
    let rec aux x =
      ...
      (* Limit updates to 1 per 10 seconds *)
      aux x
    in
    let x = Lwt_condition.wait webhook_cond in
    let thread =
...
let get_ci_refs t { Repo_id.owner; name } =
  ...
  try
    ...
    let refs =
      repo / "refs" / "edges"
      |> to_list
      |> List.map (parse_ref ~owner_name ~prefix:"refs/heads/")
      |> List.map (fun r -> (t, r))
    in
    let prs =
      repo / "pullRequests" / "edges"
      |> to_list
      |> List.map (parse_pr ~owner_name)
      |> List.map (fun r -> (t, r))
    in
    ...
    let thread =
      Lwt.catch
        ...
        (function
          | Lwt.Canceled -> Lwt.return_unit
          | ex ->
              Log.err (fun f -> f "ci_refs thread failed: %a" Fmt.exn ex);
              Lwt.return_unit)
    in
    Lwt.return (fun () ->
        Lwt.cancel thread;
...
module Commit = struct
  module Set_status = struct
    ...
    module Key = struct
      type t = { commit : Commit_id.t; context : string }
    ...
    let pp f ({ Key.commit; context }, status) =
      Fmt.pf f "Set %a/%s to %a" Commit_id.pp commit context Value.pp status
    ...
    let publish ~switch:_ t job key status =
      ...
      let body =
        `Assoc (("context", `String context) :: Value.json_items status)
      in
      get_token t >>= function
      | Error (`Msg m) -> Lwt.fail_with m
      | Ok token -> (
          ...
          in
          let uri =
            status_endpoint ~owner_name:commit.Commit_id.owner_name
              ~commit:commit.Commit_id.hash
          in
          Current.Job.log job "@[<v2>POST %a:@,%a@]" Uri.pp uri
            (Yojson.Safe.pretty_print ~std:true)
            body;
          let body =
          ...
          | err ->
              Log.warn (fun f ->
                  f "@[<v2>%a failed: %s@,%s@]" pp (key, status)
                    (Cohttp.Code.string_of_status err)
                    body);
  ...
  let set_status commit context status =
    ...
    |> let> t, commit = commit and> status = status in
...
let make_config token_file = of_oauth @@ String.trim (read_file token_file)

let cmdliner = Term.(const make_config $ token_file)
