...
let installations_changed_cond = Lwt_condition.create ()

(* Fires when the list should be updated *)

let input_installation_webhook () =
  Lwt_condition.broadcast installations_changed_cond ()
...
let access_tokens_endpoint id =
  Uri.of_string
    (Fmt.strf "https://api.github.com/app/installations/%d/access_tokens" id)
...
module Installs = Current.Var (struct
  ...
  let pp =
    Fmt.using (fun t -> Int_map.bindings t |> List.map fst) Fmt.(Dump.list int)
end)
...
type t = {
  ...
  (* Accounts which can use this app. *)
...
let http { app_id; key; _ } op uri =
  ...
  | `OK | `Created ->
      ...
      json
  | err ->
      Fmt.failwith "@[<v2>Error accessing GitHub App API at %a: %s@,%s@]"
        Uri.pp uri
    ...
    (fun () ->
      ...
      |> List.filter_map (fun json ->
             ...
             if List.mem account app.whitelist then (
               ...
               Log.warn (fun f ->
                   f "Installation %d for %S : account not on whitelist!" id
                     account);
               None ))
      |> Lwt_result.return)
    (fun ex ->
      Lwt_result.fail
        (`Msg (Fmt.strf "Failed to get GitHub installations: %a" Fmt.exn ex)))
  ...
  let rec aux () =
    ...
    | Ok ids ->
        Installs.update t.installations (fun old_map ->
            let old_map =
              match old_map with Ok x -> x | Error _ -> Int_map.empty
            in
            (* Merge in new installations. Reuse existing Apis so we don't keep refreshing tokens, etc. *)
            ids
            |> ListLabels.fold_left ~init:old_map ~f:(fun acc (iid, account) ->
    ...
    (* Wait at least 1m between updates *)
    update >>= aux
  in
  aux ()
...
let make_config app_id private_key_file whitelist =
  ...
  | Ok (`RSA key) ->
      let installations =
        Installs.create ~name:"installations" (Error (`Active `Running))
      in
...
let private_key_file =
  ...
  @@ Arg.info ~doc:"A file containing the GitHub app's RSA private key."
       ~docv:"PATH"
...
let app_id =
  ...
  @@ Arg.info ~doc:"The GitHub app's (integer) ID" ~docv:"ID"
       [ "github-app-id" ]

let whitelist =
  Arg.required
  @@ Arg.opt Arg.(some (list string)) None
  @@ Arg.info ~doc:"A comma-separated list of whilelisted GitHub accounts"
       ~docv:"ACCOUNTS"
       [ "github-account-whitelist" ]

let cmdliner = Term.(const make_config $ app_id $ private_key_file $ whitelist)
