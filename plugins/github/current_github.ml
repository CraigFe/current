...
module Metrics = struct
  ...
  let webhook_events_total =
    ...
    Counter.v_label ~label_name:"event" ~help ~namespace ~subsystem
...
let input_webhook req _body =
  ...
  Log.info (fun f ->
      f "Got GitHub event %a"
        Fmt.(option ~none:(unit "NONE") (quote string))
        event);
  Prometheus.Counter.inc_one
    (Metrics.webhook_events_total (Option.value event ~default:"NONE"));
  ( match event with
  | Some "installation_repositories" ->
      Installation.input_installation_repositories_webhook ()
