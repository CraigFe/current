...
module Key = struct
  type t = { tag : string; docker_context : string option }
  [@@deriving to_yojson]
...
module Value = struct
  ...
  let digest { image } =
    Yojson.Safe.to_string @@ `Assoc [ ("image", `String (Image.hash image)) ]
end
...
let publish ~switch auth job key value =
  ...
  | Ok () ->
      ...
      in
      Current.Process.check_output ~job cmd
...
let pp f (key, value) =
  Fmt.pf f "%a; docker push %S" Cmd.pp (tag_cmd key value) key.Key.tag
