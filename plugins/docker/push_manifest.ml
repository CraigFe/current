...
module Value = struct
  type t = { manifests : S.repo_id list }
...
let publish ~switch auth job tag value =
  ...
  | Ok () ->
      ...
      >>!= fun () -> Current.Process.exec ~switch ~job (push_cmd ~config tag)

let pp f (tag, value) = Fmt.pf f "push %s = %s" tag (Value.digest value)
