...
let pp_tag =
  Fmt.using (Astring.String.cuts ~sep:":") Fmt.(list ~sep:(unit ":@,") string)
...
module Make (Host : S.HOST) = struct
  ...
  let build ?schedule ?timeout ?(squash = false) ?label ?dockerfile ?pool ~pull
      src =
    ...
    |> let> commit = get_build_context src
       ...
       BC.get ?schedule
         { Build.pull; pool; timeout }
         { Build.Key.commit; dockerfile; docker_context; squash }
  ...
  let tag ~tag image =
    ...
    |> let> image = image in
       TC.set Tag.No_context
         { Tag.Key.tag; docker_context }
         { Tag.Value.image }
  ...
  let service ~name ~image () =
    ...
    |> let> image = image in
       SC.set Service.No_context
         { Service.Key.name; docker_context }
         { Service.Value.image }
end
