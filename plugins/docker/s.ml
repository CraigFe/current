type source = [ `No_context | `Git of Current_git.Commit.t Current.t ]
...
module type DOCKER = sig
  ...
  val pull :
    ...
    string ->
    Image.t Current.t
  (** [pull ~schedule tag] ensures that the latest version of [tag] is cached
      locally, downloading it if not.

      @param schedule
      Controls how often we check for updates. If the schedule has no
      [valid_for] limit then we will only ever pull once. *)
  ...
  val run :
    ?label:string -> Image.t Current.t -> args:string list -> unit Current.t
  ...
  (** [push image ~tag] does "docker tag image tag && docker push tag".
      ...
      @param auth
      If give, do a "docker login" using this username/password pair before
      pushing. *)

  val service :
    name:string -> image:Image.t Current.t -> unit -> unit Current.t
  (** [service ~name ~image ()] keeps a Docker SwarmKit service up-to-date. *)
end

module type HOST = sig
  val docker_context : string option
  (** The value to pass to Docker via the "--context" argument ([None] for no
      argument). *)
end
