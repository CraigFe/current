...
val record :
  ...
  unit
(** [record ~op ~key ~value ~job_id ~ready ~running ~finished ~build outcome]
    records that [key] is now set to [value], producing [outcome]. This
...
(** [lookup ~op key] returns the most recently stored result for [op] and
    [key], if any. *)
