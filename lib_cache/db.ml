...
let format_timestamp time =
  ...
  Fmt.strf "%04d-%02d-%02d %02d:%02d:%02d" (tm_year + 1900) (tm_mon + 1)
...
let db =
  lazy
    (let db = Lazy.force Current.Db.v in
     Sqlite3.exec db
       "CREATE TABLE IF NOT EXISTS cache ( op        TEXT NOT NULL, key       \
        BLOB, job_id    TEXT NOT NULL, value     BLOB, ok        BOOL NOT \
        NULL, outcome   BLOB, build     INTEGER NOT NULL, rebuild   BOOL NOT \
        NULL DEFAULT 0, ready     DATETIME NOT NULL, running   DATETIME, \
        finished  DATETIME NOT NULL, PRIMARY KEY (op, key, build))"
     |> or_fail "create table";
     let record =
       Sqlite3.prepare db
         "INSERT OR REPLACE INTO cache (op, key, job_id, value, ok, outcome, \
          ready, running, finished, build) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, \
          ?)"
     in
     let lookup =
       Sqlite3.prepare db
         "SELECT value, job_id, ok, outcome, strftime('%s', ready), \
          strftime('%s', running), strftime('%s', finished), rebuild, build \
          FROM cache WHERE op = ? AND key = ? ORDER BY build DESC LIMIT 1"
     in
     let invalidate =
       Sqlite3.prepare db
         "UPDATE cache SET rebuild = 1 WHERE op = ? AND key = ?"
     in
...
let record ~op ~key ~value ~job_id ~ready ~running ~finished ~build outcome =
  let ok, outcome =
    match outcome with Ok x -> (1L, x) | Error (`Msg m) -> (0L, m)
  in
...
let query ?op ?ok ?rebuild () =
  ...
  let query =
    Sqlite3.prepare t.db
      (Fmt.strf
         "SELECT build, ok, outcome, rebuild, value, \n\
         \                strftime('%%s', ready), \n\
         \                strftime('%%s', running), \n\
         \                strftime('%%s', finished), \n\
         \                job_id FROM cache %a ORDER BY finished DESC LIMIT 100"
