...
module Schedule = struct
  type t = { valid_for : Duration.t option }
...
let pp_duration_rough f d =
  let rec aux = function
    ...
    | (to_int, units) :: xs ->
        let i = to_int d in
        if i < 5 then aux xs else Fmt.pf f "%d %s" i units
  in
  aux durations
...
module Output (Op : S.PUBLISHER) = struct
  ...
  module Value : sig
    ...
    type t = { value : Op.Value.t; digest : string }
  ...
  type op = {
    value : Value.t;
    (* The value currently being set. *)
    switch : Current.Switch.t;
    (* Turning this off aborts the operation. *)
    job : Job.t;
    finished : unit Lwt.t;
  ...
  type output = {
    key : Op.Key.t;
    mutable build_number : int64;
    (* Number of recorded (incl failed) builds with this key. *)
    mutable ref_count : int;
    (* The number of watchers waiting for the result (for auto-cancel). *)
    mutable last_set : Current.Step.id;
    (* Last evaluation step setting this output. *)
    mutable job_id : Current.job_id option;
    (* Current or last log *)
    mutable current : string option;
    (* The current digest value, if known. *)
    mutable desired : Value.t;
    (* The value we want. *)
    mutable ctx : Op.t;
    (* The context for [desired]. *)
    mutable op :
      [ `Active of op (* The currently-running operation. *)
      | `Error of [ `Msg of string ] (* Why [desired] isn't possible. *)
      | `Finished of Op.Outcome.t
      | (* Note: if current <> desired then rebuild. *)
        `Retry
        (* Need to try again. *) ];
    mutable mtime : float;
        (* Time last operation completed (if finished or error). *)
  }
  ...
  let pp_output f output =
    ...
    | `Error (`Msg msg) -> Fmt.pf f "%a: %s" pp_desired output msg
    | `Finished _ -> Fmt.pf f "%a (completed)" pp_desired output
    | `Retry -> Fmt.pf f "%a (retry scheduled)" pp_desired output
    | `Active op ->
        if Value.equal op.value output.desired then
          Fmt.pf f "%a (in-progress)" pp_op (output.key, op.value)
        else
          Fmt.pf f "%a (stale), then %a" pp_op (output.key, op.value)
            pp_desired output
  ...
  let rec maybe_start ~config output =
    ...
    | `Active op ->
        ...
        Lwt.async (fun () ->
            ...
            @@ Error
                 (`Msg "Auto-cancelling job because it is no longer needed"))
    | `Retry ->
        invalidate_output output;
        publish ~config output
    | `Finished _ -> (
        match output.current with
        | Some current when current = Value.digest output.desired ->
  ...
  and publish ~config output =
    ...
    let op =
      { value = output.desired; switch; job; finished; autocancelled = false }
    in
          ...
          (fun () ->
            ...
            if op.autocancelled then (
              ...
              let running =
                ...
                | Lwt.Sleep when Stdlib.Result.is_ok outcome ->
              ...
              Db.record ~op:Op.id ~job_id ~key:(Op.Key.digest output.key)
                ~value:(Value.digest op.value) ~ready ~running
                ~finished:end_time ~build:output.build_number outcome;
              ...
              if output.ref_count > 0 then maybe_start ~config output;
              Lwt.wakeup set_finished ())
          (fun () -> Current.Switch.turn_off switch @@ Ok ()))
  ...
  let resolved o ~schedule ~value ~config =
    ...
    | Some job_id -> (
        let rebuild () =
          ...
          | `Active _ ->
              Log.info (fun f ->
                  f "Rebuild(%a): already rebuilding" pp_op (key, value));
              Option.get o.job_id
        in
        ...
        | Some duration ->
            let remaining_time =
              Duration.to_f duration +. o.mtime -. !Job.timestamp ()
            in
            let changed =
              if remaining_time <= 0.0 then (
                Log.info (fun f ->
                    f "Result for %a has expired" pp_op (key, value));
                invalidate key;
            ...
            @@ object
                 method pp f =
                   if remaining_time <= 0.0 then
                     ...
                     Fmt.pf f "%a will be invalid after %a" pp_op (key, value)
                       pp_duration_rough
                       (Duration.of_f remaining_time)
  ...
  let set ?(schedule = Schedule.default) ctx key value =
    ...
    let o =
      ...
      | Some o ->
          ...
          if o.last_set = step_id then (
            if changed then
              Fmt.failwith
                "Error: output %a set to different values in the same step!"
                pp_op (key, value) )
          else (
            o.last_set <- step_id;
            o.ctx <- ctx;
            if changed then (
              ...
              | `Error _ ->
                  o.op <- `Retry
                  (* Clear error when the desired value changes. *)
    ...
    | `Error e ->
        ( (Error e :> Op.Outcome.t Current_term.Output.t),
          resolved ~schedule ~value ~config o )
    | `Retry ->
        (Error (`Msg "(retry)"), resolved ~schedule ~value ~config o)
        (* (probably can't happen) *)
    | `Active op ->
        let a, changed =
          let started = Job.start_time op.job in
          if Lwt.state started = Lwt.Sleep then
            (`Ready, Lwt.choose [ Lwt.map ignore started; op.finished ])
          else (`Running, op.finished)
        in
        o.ref_count <- o.ref_count + 1;
        let cancel ~msg () =
          Lwt.async (fun () ->
              Current.Switch.turn_off op.switch @@ Error (`Msg msg))
        in
        ( Error (`Active a),
          Current.Input.metadata ?job_id:o.job_id ~changed
          @@ object
               method pp f = pp_output f o
               ...
               method release =
                 ...
                 if o.ref_count = 0 && Op.auto_cancel then
                   ...
                   | `Active op ->
                       ...
                       cancel
                         ~msg:
                           "Auto-cancelling job because it is no longer needed"
                         ()
                   | _ -> ()
             end )
...
module Make (B : S.BUILDER) = struct
  module Adaptor = struct
    ...
    let publish ~switch op job key () = B.build ~switch op job key
  ...
  let get ?schedule ctx key = set ?schedule ctx key ()
end
