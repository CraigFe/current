...
struct
  type state = Blocked | Active of Output.active | Pass | Fail of string
  ...
  type env = { next : int ref; bind : t option }

  let make_env () = { next = ref 0; bind = None }
  ...
  let blocked ~env () = make ~env (Constant None) Blocked

  let return ~env label = make ~env (Constant label) Pass

  let fail ~env msg = make ~env (Constant None) (Fail msg)

  let map_failed ~env t msg = make ~env (Map_failed t) (Fail msg)

  let state ~env t = make ~env (State t) Pass

  let catch ~env t =
    let state = match t.state with Fail _ -> Pass | _ -> t.state in
    make ~env (Catch t) state

  let active ~env a = make ~env (Constant None) (Active a)
  ...
  let simplify x =
    match (x.ty, x.bind) with Constant None, Some c -> c | _ -> x

  let pair_state a b =
    match (a.state, b.state) with
    | _, Fail m | Fail m, _ -> Fail m
    | _, (Blocked | Active _) | (Blocked | Active _), _ -> Blocked
    | Pass, Pass -> Pass

  let pair ~env a b =
    let a = simplify a in
    let b = simplify b in
    let single_context = a.bind =? b.bind && b.bind =? env.bind in
    match (single_context, a.ty, b.ty) with
    | true, Constant None, _ -> b
    | true, _, Constant None -> a
    | _ -> make ~env (Pair (a, b)) (pair_state a b)
  ...
  let set_state t ?id state =
    t.state <- state;
    match (t.ty, id) with
    | Bind_input { x; info; id = None }, Some _ ->
        t.ty <- Bind_input { x; info; id }
    | _, None -> ()
    | _ -> assert false
  ...
  let pp f x =
    ...
    let rec aux f md =
      if Id.Set.mem md.id !seen then Fmt.string f "..."
      else (
        ...
        | Constant None -> (
            ...
            | None ->
                Fmt.string f
                  (if md.state = Blocked then "(input)" else "(const)") )
        ...
        | Gate_on { ctrl; value } ->
            Fmt.pf f "%a@;>>@;gate (@[%a@])" aux value aux ctrl
        | List_map { items; fn } ->
            Fmt.pf f "%a@;>>@;list_map (@[%a@])" aux items aux fn
  ...
  type out_node = { i : int; outputs : int list }
  ...
  let pp_dot ~url f x =
    ...
    let rec aux md =
      ...
      | None ->
          let i = !next in
          incr next;
          let ctx = match md.bind with None -> [] | Some c -> aux c in
          let bg =
            match md.state with
            | Blocked -> "#d3d3d3"
            | Active `Ready -> "#ffff00"
          ...
          in
          let tooltip =
            match md.state with Fail msg -> Some msg | _ -> None
          in
          let node ?id =
            let url = match id with None -> None | Some id -> url id in
            Dot.node ~style:"filled" ~bg ?tooltip ?url f
          in
          let outputs =
            match md.ty with
            | Constant (Some l) ->
            ...
            | Bind (x, name) ->
                let inputs =
                  match x.ty with Constant None -> [] | _ -> aux x
                in
                node i name;
          ...
          outputs
    in
    Fmt.pf f "@[<v2>digraph pipeline {@,node [shape=\"box\"]@,rankdir=LR@,";
    let _ = aux x in
    Fmt.pf f "}@]@."
  ...
  let job_id t = match t.ty with Bind_input i -> i.id | _ -> None
end
