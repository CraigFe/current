...
module type INPUT = sig
  type 'a t
  (** An input that was used while evaluating a term. If the input changes, the
...
module type ANALYSIS = sig
  ...
  type t
  (** Information about the dependency graph of a term. This is useful to
      display the term's state as a diagram. *)
...
module type TERM = sig
  ...
  val state :
    'a t -> ('a, [ `Active of Output.active | `Msg of string ]) result t
  (** [state t] always immediately returns a successful result giving the
      current state of [t]. *)

  val catch : 'a t -> 'a or_error t
  (** [catch t] successfully returns [Ok x] if [t] evaluates successfully to
      [x], or successfully returns [Error e] if [t] fails with error [e]. If
      [t] is active then [catch t] will be active too. *)
  ...
  val map : ('a -> 'b) -> 'a t -> 'b t
  (** [map f x] is a term that runs [x] and then transforms the result using
      [f]. *)
  ...
  (** [component name] is used to annotate binds, so that the system can show a
      ...
      as the label for the bind in the generated dot diagrams. For convenience,
  ...
  (** [bind_input ~info f x] is a term that first runs [x] to get [y] and then
      behaves as the input [f y]. [info] is used to describe the operation in
      the analysis result. *)

  val list_map : pp:'a Fmt.t -> ('a t -> 'b t) -> 'a list t -> 'b list t
  (** [list_map ~pp f xs] adds [f] to the end of each input term and collects
      all the results into a single list.
  ...
  (** [list_seq x] evaluates to a list containing the results of evaluating
      each element in [x], once all elements of [x] have successfully
      completed. *)

  val option_seq : 'a t option -> 'a option t
  (** [option_seq None] is [Current.return None] and [option_seq (Some x)] is
      [Current.map some x]. This is useful for handling optional arguments that
      are currents. *)
  ...
  module Syntax : sig
    ...
    (** Monadic [bind]. Use this if the next part of your pipeline can only be
        determined at runtime by looking at the concrete value. Static analysis
        cannot predict what this will do until the input is ready. *)

    val ( let> ) : 'a t -> ('a -> 'b input) -> description -> 'b t
    (** [let>] is used to define a component. e.g.
        [component "my-op" |> let> x = fetch uri in ...] *)
    ...
    val ( and> ) : 'a t -> 'b t -> ('a * 'b) t
    (** Syntax for [pair]. Use this to depend on multiple terms. Note: this is
        the same as [and+]. *)
...
module type EXECUTOR = sig
  ...
  (** [run ~env f] evaluates term [f ()], returning the current output and its
      analysis. *)
end
