...
let rec read () =
  ...
  | Some x ->
      data := None;
      Lwt.return x
  | None -> Lwt_condition.wait data_cond >>= read
...
let get_watch () =
  match !w with None -> failwith "No active watch!" | Some w -> w
...
let trace step { Current.Engine.value = out; watches = inputs; _ } =
  ...
  | 2 ->
      ...
      (* Calls read immediately *)
      Lwt.pause () >>= fun () ->
      w.update ();

      (* Marks as out-of-date *)
      Lwt.pause () >>= fun () ->
      w.update ();
      Lwt.pause () >>= fun () ->
      data := Some (Ok "foo");
  ...
  | 6 ->
      ...
      assert (!w <> None);
      Lwt_condition.broadcast unwatch ();

      (* Allow first shutdown to finish *)
      Lwt.pause () >>= fun () ->
      data := Some (Ok "restart");
      Lwt_condition.broadcast data_cond ();

      (* Will re-read after shutdown *)
      Lwt.pause () >>= fun () ->
      assert (!w <> None);
      let w = get_watch () in
      Lwt.wakeup w.set_ready ();
...
let basic _switch () =
  ...
  Lwt.catch
    ...
    (function Exit -> Lwt.return_unit | ex -> Lwt.fail ex)

let tests = [ Driver.test_case_gc "basic" basic ]
