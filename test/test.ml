...
let test_v1_cancel _switch () =
  ...
  | 2 ->
      Driver.cancel
        "docker run \"image-src-123\" \"make\" \"test\" (in-progress)"
  | _ -> raise Exit
...
let test_v3 _switch () =
  ...
  | 2 ->
      ...
      Docker.complete "win-image-src-123" ~cmd:[ "make"; "test" ]
      @@ Error (`Msg "Missing DLL")
  | _ -> raise Exit
...
let test_v4 _switch () =
  ...
  | 1 -> Git.complete_clone test_commit
  | 2 ->
      Docker.complete "image-src-123" ~cmd:[ "make"; "test" ]
      @@ Error (`Msg "Failed")
  | _ -> raise Exit
...
let v5 commit =
  ...
  let bin = build src in
  let ok = test bin in
  Opam.revdeps src |> Current.gate ~on:ok
