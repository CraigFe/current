...
let settings config =
  ...
  in
  form
    ~a:[ a_action "/set/confirm"; a_method `Post ]
    [
...
let dashboard engine =
  ...
  let { Current.Engine.value; analysis = _; watches = _; jobs = _ } =
    Current.Engine.state engine
  in
  template
    [
      div [ object_ ~a:[ a_data "/pipeline.svg" ] [ txt "Pipeline diagram" ] ];
      h2 [ txt "Result" ];
