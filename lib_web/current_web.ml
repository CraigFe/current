...
type actions =
  < rebuild : (unit -> string) option ; cancel : (unit -> unit) option >
...
let render_svg a =
  ...
  | Unix.WEXITED i ->
      errorf "dot failed (exit status %d) - is graphviz installed?" i
  | Unix.WSTOPPED i | Unix.WSIGNALED i -> errorf "dot crashed (signal %d)" i
...
let handle_request ~engine ~webhooks _conn request body =
  ...
  | Lwt.Sleep -> (
      ...
      Log.info (fun f ->
          f "HTTP %s %S" (Cohttp.Code.string_of_method meth) path);
      ...
      | `GET, [ "metrics" ] ->
          ...
          Server.respond_string ~status:`OK ~headers ~body ()
      | `POST, [ "webhooks"; hook ] -> (
          match List.assoc_opt hook (webhooks : webhook list) with
  ...
  Lwt.try_bind
    ...
    (function
      | Unix.Unix_error (Unix.EADDRINUSE, "bind", _) ->
          let msg =
            Fmt.strf
              "Web-server failed.@ Another program is already using this port \
               %a."
              pp_mode mode
          in
...
let cmdliner = Term.(const make $ port)
